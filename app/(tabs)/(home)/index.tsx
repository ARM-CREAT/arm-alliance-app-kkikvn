...
export default function HomeScreen() {
  ...
  const fetchMemberCount = useCallback(async () => {
    console.log('[Home] Fetching member count');
    try {
      ...
      console.log('[Home] Member count received:', count);
      setMemberCount(typeof count === 'number' ? count : null);
    } catch (err) {
      console.warn('[Home] Failed to fetch member count (non-blocking):', err);
      setMemberCount(null);
    }
  }, []);
  ...
  const handleProgramPress = (index: number) => {
    console.log('[Home] Programme card pressed, index:', index);
    router.push('/program' as any);
  };

  const handleSeeAllProgram = () => {
    console.log('[Home] Voir tout programme pressed');
    router.push('/program' as any);
  };
